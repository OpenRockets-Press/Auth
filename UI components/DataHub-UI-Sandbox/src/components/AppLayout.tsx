...
export const AppLayout: React.FC<{ children: React.ReactNode, fullWidth?: boolean }> = ({ children, fullWidth = false }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', width: '100vw', backgroundColor: '#000000' }}>
      ...
      <header style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }} onClick={() => navigate('/')}>
            <img src={logoPath} alt="OpenRockets Logo" style={{ height: '28px', width: 'auto', filter: 'invert(1)' }} />
            ...
            {showPortalMenu && (
              <>
                ...
                <div style={{ 
                  ...
                  <button 
                    onClick={() => { navigate('/'); setShowPortalMenu(false); }}
                    style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'none', border: 'none', color: '#ffffff', cursor: 'pointer', borderRadius: '12px', textAlign: 'left' }}
                  ...
                  <button 
                    onClick={() => { navigate('/developer'); setShowPortalMenu(false); }}
                    style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'none', border: 'none', color: '#ffffff', cursor: 'pointer', borderRadius: '12px', textAlign: 'left' }}
                    onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#222'}
                    onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}
                  >
                  ...
                  <button 
                    onClick={() => { navigate('/admin'); setShowPortalMenu(false); }}
                    style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'none', border: 'none', color: '#ffffff', cursor: 'pointer', borderRadius: '12px', textAlign: 'left' }}
                    onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#222'}
                    onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}
                  >
